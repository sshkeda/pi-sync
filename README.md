...
Both terminals attach to the same live session/sync channel and render the same conversation/tool/status stream.
...
- auto-starts a fresh per-session/same-sync `pi-sync-host` on `session_start`
...
- exports primary `PI_SYNC_*` identity variables, still reads legacy `PI_LANE_*`, and ships `pisync`/`pi-sync` CLIs plus a deprecated `pil` alias
...
- `test/native-live-sync.test.mjs` proves pi-sync publishes native updates from one same-session terminal to another and keeps tree navigation on the main sync channel without hidden branch channels.
